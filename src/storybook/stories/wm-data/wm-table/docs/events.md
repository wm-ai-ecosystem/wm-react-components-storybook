...
<details open>
  <summary>Callback Events</summary>
    <div>
        ...
        | `onRowSelect` | This event will be called when a row from Data Table is selected. **Note**: Row selection will happen on click of a cell. So, cell element is present in $event. |
        | `onRowDeselect` | This event will be called when a row from Data Table is deselected. **Note**: Row deselection will happen on click of a cell. So, cell element is present in $event. |
        ...
        | `onBeforeRecordInsert` | This event handler is triggered before a new record is inserted into the underlying data entity. i.e., **onBeforeRowInsert** |
        | `onRecordInsert` | This event handler is triggered after a new record is successfully inserted into the underlying data entity. i.e., **onRowInsert** |
        | `onBeforeRecordUpdate` | This event handler is triggered before a record is updated in the underlying data entity. i.e., **onBeforeRowUpdate** |
        | `onRecordUpdate` | This event handler is triggered after a record is updated in the underlying data entity. i.e., **onRowUpdate** |
